...
def index(request):
    ...
    voted_restaurants = vote_logic.get_voted_restaurants_from_user(request.user) if request.user.is_authenticated else None
    supporters = vote_logic.get_choice_of_the_day_supporters(choice_of_the_day)
    not_voted = user_logic.get_users_that_not_voted_yet()
...
def vote(request):
    ...
    if restaurant_names:
        ...
        absence_logic.delete_old_absence_for_user(user)
