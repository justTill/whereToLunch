...
class RestaurantLogic:
    ...
    def get_votes_for_restaurant_with_id(self, restaurant_id):
        logger.info("get votes for restaurants with id: %s" % restaurant_id)
        ...
        if votes:
            ...
            logger.info("there is no vote for that restaurant with id: %s" % restaurant_id)
            return 0

    def get_restaurants_with_names(self, restaurant_names):
        logger.info("get restaurants with names: %s" % restaurant_names)
    ...
    def sort_restaurant_dict(self, restaurant_dict):
        logger.info("sorting this restaurants dict after votes: %s" % restaurant_dict)
